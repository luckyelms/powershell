<#
    Ver: 1.1:
        - supercedes registerdns1.0.
        - Exclusive to UH, Wh will have an identical but seperate script   
        - Ver 1.0 works fine but does not check if user is on Jumia Wi-fi
        - if host is not connected to Jumia wifi, do nothing..
    Tasks:
        1. Check if host is connected to JUMIAH-UH(2) > DONE
        2. if yes: check if DNS & search domains are correct, if not set > DONE
        3. Then register IP with Bind > DONE
    
	$ErrorActionPreference = "continue"
	Status: Started 16-07-23 @1350 > in-progress...
            17.07.23 @1424: Ready 4deployement
	Logs:
...
if ($wifiInterface) 
{
    ...
    if ($ssid -like "JUMIA-UH" -or $ssid -like "JUMIA-UH2")
    {
        ...
        if ($dnsSuffix -eq 'uh.lab' -and $dnsServers -contains '10.8.8.15' -and $dnsServers -contains '8.8.8.8') 
        {
            ...
            #---------- log to remote host 
            # line below is a test code. when done delete it:
            Add-Content -Path "C:\conf\WifiLogs.txt" -Value "I am at the try block, goint to open a session"
            ...
            try
            {
                ...
                {
                    # line below is a test code. when done delete it:
                    Add-Content -Path "C:\conf\WifiLogs.txt" -Value "session has been opened & data is going 2b written.."
                    ...
                    # line below is a test code. when done delete it:
                    Add-Content -Path "C:\conf\WifiLogs.txt" -Value "session closed..."
                }
                else
                {
                    Add-Content -Path "C:\conf\WifiLogs.txt" -Value "Session failed to open..."
            ...
            {
                Add-Content -Path "C:\conf\WifiLogs.txt" -Value "at the catch block..."
        ...
        {
            # delete the line below when all is good
            Add-Content -Path "C:\conf\WifiLogs.txt" -Value "DNS configuration is not as expected @the else block..."
